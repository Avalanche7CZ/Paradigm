    ...
    public static void onChatMessage(ServerChatEvent event) {
        ...
        if (mentionEveryone) {
            if (!canMentionEveryone(sender)) {
                sender.sendSystemMessage(Component.literal("§cYou are mentioning everyone too frequently. Please wait a while."));
            ...
            if (!hasPermission && !hasPermissionLevel) {
                sender.sendSystemMessage(Component.literal("§cYou do not have permission to mention everyone."));
                event.setCanceled(true);
                return;
            }
            ...
            for (ServerPlayer player : players) {
                ...
                if (message.contains(mention)) {
                    if (!canMentionPlayer(sender, player)) {
                        sender.sendSystemMessage(Component.literal("§cYou are mentioning players too frequently. Please wait a while."));
                        event.setCanceled(true);
                        return;
                    }
                    ...
                    if (!hasPermission && !hasPermissionLevel) {
                        sender.sendSystemMessage(Component.literal("§cYou do not have permission to mention players."));
                        event.setCanceled(true);
                        return;
                    }
