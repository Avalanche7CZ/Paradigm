...
public class NeoForgeCommandBuilder implements ICommandBuilder {
    ...
    public ICommandBuilder requires(Predicate<ICommandSource> requirement) {
        if (currentBuilder instanceof LiteralArgumentBuilder<?> lit) {
            ...
            this.currentBuilder = b.requires(source -> requirement.test(MinecraftCommandSource.of(source)));
    ...
    public ICommandBuilder suggests(SuggestionProvider provider) {
        if (currentBuilder instanceof RequiredArgumentBuilder<?, ?> req) {
            @SuppressWarnings("unchecked")
            RequiredArgumentBuilder<CommandSourceStack, ?> b = (RequiredArgumentBuilder<CommandSourceStack, ?>) req;
            this.currentBuilder = b.suggests((ctx, builder) -> {
                ICommandContext pc = new NeoForgeCommandContext(ctx);
    ...
    public ICommandBuilder then(ICommandBuilder child) {
        Object childBuilder = child != null ? child.build() : null;
        ...
        if (currentBuilder instanceof LiteralArgumentBuilder<?> lit) {
            ...
            if (childBuilder instanceof LiteralArgumentBuilder<?> childLit) {
                @SuppressWarnings("unchecked")
                LiteralArgumentBuilder<CommandSourceStack> c2 = (LiteralArgumentBuilder<CommandSourceStack>) childLit;
                this.currentBuilder = b.then(c2);
            } else if (childBuilder instanceof RequiredArgumentBuilder<?, ?> childReq) {
                @SuppressWarnings("unchecked")
                RequiredArgumentBuilder<CommandSourceStack, ?> c2 = (RequiredArgumentBuilder<CommandSourceStack, ?>) childReq;
                this.currentBuilder = b.then(c2);
            }
        } else if (currentBuilder instanceof RequiredArgumentBuilder<?, ?> req) {
            @SuppressWarnings("unchecked")
