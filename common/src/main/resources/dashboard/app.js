...
function renderActions(actions, path) {
  const rows = actions.map((action, index) => {
    ...
    else if (type === 'await_input') fields = `<label>Prompt lines<textarea data-action-field="text">${esc((action.text || []).join('\n'))}</textarea></label><div class="compact-form"><label>Input key<input data-action-field="key" value="${attr(action.key || 'input')}" placeholder="input"></label><label>Timeout (seconds)<input data-action-field="timeout" type="number" min="1" max="3600" value="${attr(action.timeout ?? 60)}"></label></div><h3>On Success</h3><div>${renderActions(action.on_success || [], `${actionPath}.on_success`)}</div><button data-add-action-path="${actionPath}.on_success">Add Success Action</button><h3>On Failure (cancel or timeout)</h3><div>${renderActions(action.on_failure || [], `${actionPath}.on_failure`)}</div><button data-add-action-path="${actionPath}.on_failure">Add Failure Action</button>`;
...
function normalizeAction(action) {
  ...
  if (type === 'await_input') Object.assign(action, { key: 'input', timeout: 60, text: ['Type your message in chat.'], on_success: [], on_failure: [] });
