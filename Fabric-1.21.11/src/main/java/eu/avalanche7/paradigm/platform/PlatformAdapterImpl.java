...
public class PlatformAdapterImpl implements IPlatformAdapter {
    ...
    public boolean hasVanillaPermissionLevel(IPlayer player, int level) {
        ...
        var required = switch (level) {
            case 1 -> DefaultPermissions.MODERATORS;
            case 2 -> DefaultPermissions.GAMEMASTERS;
            case 3 -> DefaultPermissions.ADMINS;
            default -> DefaultPermissions.OWNERS;
        };
        return mp.getHandle().getPermissions().hasPermission(required);
    }
    ...
    public void playSound(IPlayer player, String soundId, String category, float volume, float pitch) {
        ...
        if (soundEvent != null) {
            ...
            handle.networkHandler.sendPacket(new PlaySoundS2CPacket(
                    ...
                    volume, pitch, handle.getEntityWorld().getRandom().nextLong()
            ));
        }
    }
    ...
    public void executeCommandAs(ICommandSource source, String command) {
        ...
        if (orig instanceof ServerCommandSource scs) {
            server.getCommandManager().parseAndExecute(scs, command);
        } else {
    ...
    public boolean setMovementSpeed(IPlayer player, double baseValue) {
        ...
        var attribute = mp.getHandle().getAttributeInstance(EntityAttributes.MOVEMENT_SPEED);
    ...
    public boolean enchantMainHand(IPlayer player, String enchantmentId, int level) {
        ...
        var enchantment = server.getRegistryManager()
    ...
    public Integer getHighestBlockY(IPlayer player) {
        if (!(player instanceof MinecraftPlayer mp)) return null;
        ServerPlayerEntity handle = mp.getHandle();
        return ((ServerWorld) handle.getEntityWorld()).getTopY(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES,
    ...
    public java.util.Optional<Double> findSafeRtpY(IPlayer player, double x, double z) {
        ...
        ServerWorld world = (ServerWorld) mp.getHandle().getEntityWorld();
    ...
    public boolean jumpPlayerForward(IPlayer player, int distance) {
        ...
        handle.teleport(handle.getX() + look.x * distance, handle.getY() + look.y * distance,
                handle.getZ() + look.z * distance, false);
        return true;
    }
    ...
    public boolean teleportPlayer(IPlayer player, PlayerDataStore.StoredLocation location) {
        ...
        if (id == null) return false;
        ServerWorld target = server.getWorld(net.minecraft.registry.RegistryKey.of(RegistryKeys.WORLD, id));
        if (target == null) return false;
        return mp.getHandle().teleport(target, location.getX(), location.getY(), location.getZ(), java.util.Set.of(),
    ...
    public String getPlayerRemoteAddress(IPlayer player) {
        ...
        return String.valueOf(mp.getHandle().networkHandler.getConnectionAddress());
    }
    ...
    public boolean isPlayerInArea(IPlayer player, String worldId, List<Integer> corner1, List<Integer> corner2) {
        ...
        if (!mp.getHandle().getEntityWorld().getRegistryKey().equals(targetWorldKey)) {
            return false;
        }

        Vec3d pos = new Vec3d(mp.getHandle().getX(), mp.getHandle().getY(), mp.getHandle().getZ());
        double pX = pos.getX();
        double pY = pos.getY();
        double pZ = pos.getZ();
    ...
    public Object createStyleWithClickEvent(Object baseStyle, String action, String value) {
        ...
        ClickEvent clickEvent = switch (action != null ? action.toUpperCase() : "") {
            case "OPEN_URL" -> {
                String url = val.startsWith("http://") || val.startsWith("https://") ? val : "https://" + val;
                yield new ClickEvent.OpenUrl(java.net.URI.create(url));
            }
            case "RUN_COMMAND", "RUN_CMD", "RUN_COMMAND_ALT" -> new ClickEvent.RunCommand(val);
            case "SUGGEST_COMMAND" -> new ClickEvent.SuggestCommand(val);
            case "CHANGE_PAGE" -> {
                int page;
                try { page = Integer.parseInt(val); } catch (NumberFormatException e) { page = 1; }
                yield new ClickEvent.ChangePage(page);
            }
            case "COPY_TO_CLIPBOARD" -> new ClickEvent.CopyToClipboard(val);
            default -> new ClickEvent.SuggestCommand(val);
        };
    ...
    public Object createStyleWithHoverEvent(Object baseStyle, Object hoverText) {
        ...
        return style.withHoverEvent(new HoverEvent.ShowText(hover));
    }
    ...
    public void registerCommand(eu.avalanche7.paradigm.platform.Interfaces.ICommandBuilder builder) {
        ...
        if (built instanceof com.mojang.brigadier.builder.LiteralArgumentBuilder<?> lit) {
            ...
            CommandDispatcher<ServerCommandSource> resolvedDispatcher = dispatcher;
            ...
            CommandPriority.RootRegistration registration = CommandPriority.registerRootLiteral(
                    resolvedDispatcher, normalizedRoot, () -> resolvedDispatcher.register(literalBuilder));
