...
public class Paradigm {
    ...
    // NOTE: These are legacy static helpers; renamed to avoid clashing with ParadigmAPI.ParadigmAccessor methods.
    ...
    public Paradigm(FMLJavaModLoadingContext ctx) {
        ...
        ParadigmAPI.setInstance(new ParadigmAPI.ParadigmAccessor() {
            ...
            public List<ParadigmModule> getModules() {
                return Paradigm.getModules();
            }

            @Override
            public Services getServices() {
                return Paradigm.getServices();
            }
    ...
    private Services initialize() {
        IConfig platformConfig = new ForgeConfig();

        DebugLogger debugLogger = new DebugLogger(null);

        MainConfigHandler.init(platformConfig, debugLogger);
        debugLogger = new DebugLogger(MainConfigHandler.getConfig());

        AnnouncementsConfigHandler.init(platformConfig, debugLogger);
        MOTDConfigHandler.init(platformConfig, debugLogger);
        MentionConfigHandler.init(platformConfig, debugLogger);
        RestartConfigHandler.init(platformConfig, debugLogger);
        ChatConfigHandler.init(platformConfig, debugLogger);
        CooldownConfigHandler.init(platformConfig, debugLogger);
        ...
        IPlatformAdapter platformAdapter = new PlatformAdapterImpl(null, placeholders, taskScheduler, debugLogger);

        PermissionsHandler permissionsHandler = new PermissionsHandler(LOGGER, cmConfig, debugLogger, platformAdapter);
        if (platformAdapter instanceof PlatformAdapterImpl pai) {
            pai.setPermissionsHandler(permissionsHandler);
        }
    ...
    public void onRegisterCommands(RegisterCommandsEvent event) {
        try {
            if (services.getPlatformAdapter() instanceof eu.avalanche7.paradigm.platform.PlatformAdapterImpl pai) {
